...
export default function TaskList({ loading, error, tasks, onEdit, onDelete, onRetry }) {
  if (loading) return <LoadingSpinner />;
  if (error) return <ErrorMessage message={error} onRetry={onRetry} />;
  if (!tasks?.length) return <p className="empty">No task yet.</p>;

  return (
    <div className="task-list">
      {tasks.map((item) => (
        <TaskItem key={item.id} task={item} onEdit={onEdit} onDelete={onDelete} />
      ))}
    </div>
  );
}
